...
resource "aws_route_table" "web_1" {
  vpc_id = aws_vpc.this.id
...
resource "aws_route_table" "web_2" {
  vpc_id = aws_vpc.this.id
...
resource "aws_route_table_association" "web_1" {
  ...
  route_table_id = aws_route_table.web_1.id
}

resource "aws_route_table_association" "web_2" {
  subnet_id      = aws_subnet.web_2.id
  route_table_id = aws_route_table.web_2.id
}
...
resource "aws_security_group" "alb" {
  ...
  egress {
    ...
    to_port     = 65535
    protocol    = "tcp"
    cidr_blocks = ["10.10.10.0/24", "10.10.11.0/24"]
  }
...
resource "aws_route53_record" "cafe_www_onprem" {
  zone_id = aws_route53_zone.this.zone_id
...
resource "aws_route53_record" "cafe_www_alb" {
  zone_id = aws_route53_zone.this.zone_id
  name    = "www.${var.domain_name}"
  type    = "A"
